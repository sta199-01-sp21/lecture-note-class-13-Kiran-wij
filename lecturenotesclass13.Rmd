...
Higher varaiability, may not be exactly the true population parameter
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-Kiran-wij/manhattan.csv")
```
...
**Interpretation**:
There is a 95% chance that the true population parameter lies within this 
confidence interval (given the i.i.d assumptions we have made about the sample).
...
manhattan %>% 
  ...
  get_ci(level = 0.95)
```
There is a 95% chance that the true median price for a one bedroom apartment in
manhattan is between 2162 and 2875.
...
- Population: Mice
- Parameter of interest: proportion of those not on treatment that died
- Sample: 3-10 day survival of mice randomized to different treatment doses
- Sample size: 522
...
**Interpretation**:
There is a 95% chance that the true proportion of mice not on the treatment who
died was between 0.690 and 0.797
...
```{r practice_2_ci}
ci_mice_died2 <- get_ci(boot_prop2, level = .90)
ci_mice_died2
```
...
visualise(boot_prop2) +
  shade_ci(ci_mice_died2)
```


Give an interpretation of your result.
There is a 95% chance that the true proportion of mice on the treatment who
died was between 0.552 and 0.651.
